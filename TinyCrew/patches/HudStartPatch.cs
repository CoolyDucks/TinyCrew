...
public static class HudStartPatch
{
    ...
    public static void Postfix(HudManager __instance)
    {
        ...
        var newCamObj = new GameObject("ZoomCamera");
        var newCamTransform = newCamObj.transform;
        newCamTransform.parent = mainCam.transform;
        newCamTransform.localPosition = Vector3.zero;
        newCamTransform.localScale = Vector3.one;
    ...
    {
        if (!_shadowCollab || !_cam) return;
        _shadowCollab.ShadowCamera.aspect = _cam.aspect;
        var val = _shadowCollab.ShadowCamera.orthographicSize = _cam.orthographicSize = _defaultOrthographicSize / TinyCrewPlugin.ScaleMod;
